...
export default function DeliverySale({ sections, shops, bankAccounts, nextInvoiceNo, currentDate, user }: SaleProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Deliveries', href: route('delivery.index') },
        { title: 'Direct Sale', href: route('delivery.delivery-sales.create') },
    ];
    ...
    const addPrinter = (printer: any) => {
        const existing = data.items.find(i => i.serial_number && i.serial_number === printer.serial_number);
        if (existing) {
    ...
    const addItem = (item: StockItem & { inputQuantity?: number }) => {
        ...
        const existing = data.items.find(i => i.itm_ky === item.itm_ky && i.batch_no === item.batch_no);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('delivery.delivery-sales.store'), {
            onSuccess: () => {
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col">
                                            <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                                                <Truck className="h-5 w-5 text-vismass-blue" />
                                                Direct Delivery Sale
                                            </h2>
                                            <p className="text-xs text-gray-500">Field-ready POS for direct inventory sales</p>
                                        </div>
                                        <button
                                            type="button"
                                        ...
                                        <div className="mb-2 border-b border-slate-100 pb-1 flex items-center justify-between">
                                            ...
                                            <div className="flex gap-4">
                                                <div>
                                                    ...
                                                    <span className="text-sm font-bold font-mono text-gray-700">{nextInvoiceNo}</span>
                                                </div>
                                                <div>
                                                    <span className="text-[10px] text-gray-400 uppercase tracking-wider block">Date</span>
                                                    <span className="text-sm font-bold font-mono text-gray-700">{currentDate}</span>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="flex flex-col space-y-4">
                                        ...
                                        <div className="grid grid-cols-5 gap-2 sm:gap-3 mb-2">
                                            {[
                                                ...
                                                <button
                                                    key={mode.id}
                                                    type="button"
                                                    className={`flex flex-col items-center justify-center gap-1 p-2 h-16 rounded-lg border-2 transition-all duration-200 ${data.payment_mode === mode.id
                                                            ? 'bg-blue-50 border-vismass-blue text-vismass-blue shadow-sm'
                                                            : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50 hover:border-slate-300'
                                                        }`}
                                                    onClick={() => {
                                                        setData('payment_mode', mode.id as any);
                                        ...
                                        <div className="mt-auto pt-2">
                                            <button
                                                className={`w-full h-14 text-base font-bold rounded-xl text-white shadow-lg transition-all duration-200 flex items-center justify-center gap-2 ${processing || data.items.length === 0
                        ...
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between items-center">
                                    ...
                                    <button
                                        type="button"
                                        className="text-[10px] font-bold text-vismass-blue hover:text-blue-700 hover:underline bg-blue-50 px-1 py-0.5 rounded-md"
                                        onClick={() => setData('cash_amount', totalAmount.toString())}
                                    >
                                        Exact
                                    </button>
