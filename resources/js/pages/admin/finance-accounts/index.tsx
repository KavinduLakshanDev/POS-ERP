...
interface Props {
    ...
    deliveryPettyCashBalance?: number;
}

export default function FinanceAccountIndex({ financeAccounts, filters, flash, pettyCashBalance, deliveryPettyCashBalance }: Props) {
    const { hasPermission } = usePermission();
    const [searchTerm, setSearchTerm] = useState(filters.search || '');
    const [selectedStatus, setSelectedStatus] = useState(filters.status || '');
    const [selectedMainCategory, setSelectedMainCategory] = useState(filters.main_category || '');
    const [toggleModal, setToggleModal] = useState<{ show: boolean; account: FinanceAccount | null }>({
        show: false,
        account: null,
    });
    const [deleteModal, setDeleteModal] = useState<{ show: boolean; account: FinanceAccount | null }>({
        show: false,
        account: null,
    });
    ...
    const groupedAccounts = financeAccounts.data.reduce((acc, current) => {
    ...
    const handleToggleStatus = (account: FinanceAccount) => {
    ...
    const confirmToggle = () => {
        if (toggleModal.account) {
            ...
            router.patch(`/admin/finance-accounts/${toggleModal.account.id}/toggle-status`, {}, {
                onSuccess: () => {
    ...
    const confirmDelete = () => {
        if (deleteModal.account) {
            ...
            router.delete(`/admin/finance-accounts/${deleteModal.account.id}`, {
                onSuccess: () => {
    ...
    }, [searchTerm, selectedStatus, selectedMainCategory]);
    ...
    // Calculate stats
    const totalAccounts = financeAccounts.data.length;
    const activeAccounts = financeAccounts.data.filter(account => account.status).length;
    const inactiveAccounts = financeAccounts.data.filter(account => !account.status).length;
    ...
    const getAccountTypeBadge = (type: string) => {
        const colors = {
            ...
            qr_payment: 'bg-indigo-100 text-indigo-800',
            petty_cash: 'bg-amber-100 text-amber-800',
            delivery_petty_cash: 'bg-orange-100 text-orange-800',
        };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <header className="bg-gradient-to-r from-vismass-blue to-vismass-grey shadow">
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between py-4">
                            ...
                            <Link
                                href="/admin/finance-accounts/create"
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                                    <div>
                                        <h3 className="text-base font-semibold text-white">
                                            {t('Finance Accounts List')}
                                        </h3>
                                        <p className="text-white/80 text-xs mt-0.5">
                                            {t('View and manage all finance accounts')}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-4">
                                    ...
                                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="relative">
                                                ...
                                                <input
                                                    type="text"
                                                    placeholder={t('Search finance accounts...')}
                                                    value={searchTerm}
                                ...
                                <div className="space-y-4">
                                    <div className="md:hidden space-y-4">
                                        {financeAccounts.data.length === 0 ? (
                                            <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center">
                                                <Building2 className="mx-auto h-12 w-12 text-slate-400" />
                                                <h3 className="mt-2 text-sm font-medium text-slate-900">{t('No finance accounts')}</h3>
                                                <p className="mt-1 text-sm text-slate-500">
                                                    {t('Get started by creating a new finance account.')}
                                                </p>
                                                <div className="mt-6">
                                                    <Link
                                                        href="/admin/finance-accounts/create"
                                            ...
                                            Object.entries(groupedAccounts).map(([category, accounts]) => (
                                                <div key={`mobile-group-${category}`} className="space-y-4">
                                                    <h4 className="font-semibold text-slate-700 capitalize border-b border-slate-200 pb-2">{category}</h4>
                                                    {accounts.map((account) => (
                                                        <div key={account.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                                            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                                                <div className="min-w-0">
                                                                    <div className="text-sm font-semibold text-slate-900 truncate">{account.account_name}</div>
                                                                </div>
                                                            ...
                                                            <div className="mt-3 grid grid-cols-1 gap-3 text-xs text-slate-600 sm:grid-cols-2">
                                                                <div>
                                                                    <div className="font-medium text-slate-900">{t('Main Category')}</div>
                                                                    <div className="capitalize">{account.main_category}</div>
                                                                </div>
                                                                <div>
                                                                    <div className="font-medium text-slate-900">{t('Type')}</div>
                                                                    <div>{getAccountTypeBadge(account.account_type)}</div>
                                                                </div>
                                                                <div className="text-left sm:text-right">
                                                                    <div className="font-medium text-slate-900">{t('Balance')}</div>
                                                                    <div>Rs {account.current_balance?.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                                                                </div>
                                                            </div>
                                                            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
                                                                {hasPermission('finance_accounts.view') && (
                                                                    <Link
                                                                        href={`/admin/finance-accounts/${account.id}`}
                                                                ...
                                                                { hasPermission('finance_accounts.edit') && (
                                                                    <Link
                                                                        href={`/admin/finance-accounts/${account.id}/edit`}
                                                                ...
                                                                {/* { hasPermission('finance_accounts.delete') && (
                                                                    <button
                                                                        onClick={() => handleDelete(account)}
                                                                        className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-100 sm:w-auto transition-colors"
                                                                    >
                                                                ...
                                                                { hasPermission('finance_accounts.edit') && (
                                                                    <button
                                                                        ...
                                                                        className={`inline-flex w-full items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-medium ${account.status
                                                                    ...
                                                                    >
                                                                        ...
                                                                        {account.status ? t('Deactivate') : t('Activate')}
                                                                    </button>
                                                                )}
                                                            </div>
                                                        </div>
                                        ...
                                        <table className="min-w-full divide-y divide-slate-200">
                                            <thead className="bg-gradient-to-r from-sky-50 to-blue-50">
                                                ...
                                                {financeAccounts.data.length === 0 ? (
                                                    ...
                                                    Object.entries(groupedAccounts).map(([category, accounts]) => (
                                                        <tbody key={`desktop-group-${category}`} className="bg-white divide-y divide-slate-200">
                                                            <tr className="bg-slate-50/80">
                                                                <td colSpan={4} className="px-4 py-3 text-sm font-semibold text-slate-700 capitalize border-b border-slate-200">
                                                                    {category}
                                                                </td>
                                                            </tr>
                                                            {accounts.map((account) => (
                                                                <tr key={account.id} className="hover:bg-slate-50 transition-colors">
                                                                    ...
                                                                    <td className="px-4 py-2.5 whitespace-nowrap">
                                                                        ...
                                                                        <div className="text-xs text-slate-500">
                                                                            Rs {account.current_balance?.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                                        </div>
                                                                    </td>
                                                                    <td className="px-4 py-2.5 whitespace-nowrap">
                                                                        {getStatusBadge(account.status)}
                                                                    ...
                                                                    <td className="px-4 py-2.5 whitespace-nowrap text-xs font-medium">
                                                                        <div className="flex items-center space-x-2">
                                                                            ...
                                                                            { hasPermission('finance_accounts.edit') && (
                                                                                <button
                                                                                    onClick={() => handleToggleStatus(account)}
                                                                                    className={`inline-flex items-center p-1.5 rounded-md transition-colors ${account.status
                                                                                        ? 'text-red-600 hover:text-red-800 hover:bg-red-50'
                                                                                        : 'text-green-600 hover:text-green-800 hover:bg-green-50'
                                                                                        }`}
                                                                                    title={account.status ? t('Deactivate') : t('Activate')}
                                                                                >
                                                                                    {account.status ? (
                                                                                        <PowerOff className="h-4 w-4" />
                                                                                    ) : (
                                    ...
                                    {/* Pagination */}
                                    {financeAccounts.meta && financeAccounts.meta.last_page > 1 && (
                                        <div className="bg-white px-4 py-3 border-t border-slate-200 sm:px-6">
                                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                                <div className="text-sm text-slate-700">
                                                    {t('Showing')} {financeAccounts.meta.from} {t('to')} {financeAccounts.meta.to} {t('of')} {financeAccounts.meta.total} {t('results')}
                                                </div>
                                                <div className="flex flex-wrap gap-1">
                                                    {financeAccounts.links.map((link, index) => (
                                                        <Link
                                                            key={index}
                                                            href={link.url || '#'}
                            ...
                            {toggleModal.account && (
                                <>
                                    ...
                                    <span className="font-semibold text-gray-900">
                                        {toggleModal.account.status ? t('deactivate') : t('activate')}
                                    </span>{' '}
            ...
            <AlertDialog open={deleteModal.show} onOpenChange={(open) => {
                if (!open) setDeleteModal({ show: false, account: null });
            }}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>{t('Delete Finance Account')}</AlertDialogTitle>
                        <AlertDialogDescription>
                            {deleteModal.account && (
                                <>
                                    {t('Are you sure you want to delete the finance account')} "{deleteModal.account.account_name}"?{' '}
